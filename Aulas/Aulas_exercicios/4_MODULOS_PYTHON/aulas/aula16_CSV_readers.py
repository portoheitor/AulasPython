...
Um exemplo de um arquivo CSV pode ser:
 Nome,Idade,Endereço
 Heitor Ribeiro,32,"Av Brasil, 21, Centro"
...
CAMINHO_ABSOLUTO = Path(__file__).parent / "aula16_CSV.csv"

# Leitura formato Dict
with open(CAMINHO_ABSOLUTO, 'r') as arquivo:
    leitor = csv.DictReader(arquivo)
    # next(leitor)#remove/pula a primeira linha (indices)

    for linha in leitor:
        # print(linha['Nome'], linha['Idade'], linha['Endereco'])
        print(linha)

#Leitura em linhas com acesso aos valores[]
# with open(CAMINHO_ABSOLUTO, 'r') as arquivo:
